...
def get_active_vpn_users():
    """Собирает входящие IP клиентов и исходящие IP сайтов (Поддерживает TCP, UDP и Docker-прокси)"""
    
    # Добавлен docker-proxy, так как Outline работает через него
    ...
    # Собираем локальные IP-адреса, чтобы отличать свои от чужих
    local_ips = set()
    for interface, snics in psutil.net_if_addrs().items():
        for snic in snics:
            if snic.family == socket.AF_INET:
    ...
    for conn in psutil.net_connections(kind='inet'):
        if conn.pid in proxy_pids and conn.raddr:
            ...
            # Игнорируем внутренние системные адреса (127.x.x.x) и локальные IP хоста
            if remote_ip in local_ips or remote_ip.startswith('127.') or remote_ip.startswith('::1'):
                continue
                
            # Игнорируем "докеровские" внутренние адреса (обычно 172.17.x.x - 172.31.x.x)
            # Это трафик между docker-proxy и самим контейнером shadowbox
            if remote_ip.startswith('172.') and local_ip.startswith('172.'):
                continue
                
            # ОПРЕДЕЛЯЕМ НАПРАВЛЕНИЕ:
            # Если локальный порт совпадает с портом LISTEN, значит к нам стучится клиент (Входящее)
            # Также для UDP-трафика (Shadowsocks/Outline) статус ESTABLISHED не применяется, 
            # поэтому мы считаем любое UDP-соединение к порту сервера - входящим от клиента.
